...
def solve_plan_lp(p, Ls, Gs, E0, q_old=None, lam=0.0, alpha=0.95,
    ...
    for s in range(S):
        for k in range(4):
            ...
            if k == 0:
                ...
                # 末端价值按等权期望归一：总末端系数为 eps_soc + v_term，
                # 不随情景数 S、A/B/C 模型或复制情景改变。
    ...
    b_ub = np.array(bu)
    def _lp(method, options):
        return linprog(c_vec, A_ub=A_ub, b_ub=b_ub, A_eq=A_eq, b_eq=b_eq,
                       bounds=np.stack([ub_l, ub_u], axis=1), method=method, options=options)

    # 2026-09-12 健壮性修正（求解阶梯）：HiGHS 在**退化算例**上偶发返回
    # "HiGHS Status 15: model_status Unknown / primal_status Feasible"
    # （实测触发于问题4 口径B A 轨 di=132 t=107、储能恰为上限 10800 kWh），
    # 同一实例换用单纯形/内点法即可正常返回最优解。故：
    #   ① 先按原样 highs；仅当**首次未成功**时才依次重试 highs-ds / highs-ipm / highs(无 presolve)；
    #   ② 任一尝试返回非零状态但**原始解通过独立可行性校验**（不等式/等式/上下界 ≤1e-6）时接受，
    #      目标值由 c_vec @ x 重算，不沿用求解器报告值；
    #   ③ 全部尝试失败则保持原行为抛错。
    # 对**首次即成功**的求解，本改动逐位无影响（不会进入重试分支）。
    ok, x_acc, res, tries = False, None, None, 0
    for method, options in (("highs", {}), ("highs-ds", {}), ("highs-ipm", {}),
                            ("highs", {"presolve": False})):
        tries += 1
        r_try = _lp(method, options)
        if res is None:
            res = r_try
        ok_try, x_try = bool(r_try.success), None
        if ok_try:
            x_try = np.asarray(r_try.x, float)
        elif r_try.x is not None and r_try.status not in (1, 2, 3):   # 1/2/3 = 迭代上限/不可行/无界
            xc = np.asarray(r_try.x, float)
            viol = 0.0
            if A_ub.shape[0]:
                viol = max(viol, float(np.max(A_ub @ xc - b_ub)))
            if A_eq.shape[0]:
                viol = max(viol, float(np.max(np.abs(A_eq @ xc - b_eq))))
            viol = max(viol, float(np.max(ub_l - xc)), float(np.max(xc - ub_u)))
            if viol <= 1e-6:
                ok_try, x_try = True, xc
        if ok_try:
            ok, x_acc, res = True, x_try, r_try
            break
    out = {"status": 0 if ok else res.status,
           "msg": (res.message + f" (第 {tries} 次尝试：可行性校验通过的原始解被接受)")
                  if (ok and not res.success) else res.message,
           "obj": float(c_vec @ x_acc) if ok else np.nan,
           "accepted_unknown_status": bool(ok and not res.success),
           "solve_tries": tries}
    if ok:
        x = x_acc
